...
__global__ void MC_inner_k(int P1, int P2, float St, float _t, int It, float dt, float B, float K, int L, int M, int Ntraj, TabSeedCMRG_t *pt_cmrg, float *option_price, int index_rng){
	...
	CMRG_get_d(&a0, &a1, &a2, &a3, &a4, &a5, pt_cmrg[0][index_rng][gb_index_x]);
	...
	CMRG_set_d(&a0, &a1, &a2, &a3, &a4, &a5, pt_cmrg[0][index_rng][gb_index_x]);
}
...
__global__ void MC_outer_k(int P1, int P2, float x_0, float dt, float B, float K, int L, int M, int Ntraj, TabSeedCMRG_t *pt_cmrg, float *option_price, float *sum, float *option_prices, float *stocks, int *It){
  ...
  CMRG_get_d(&a0, &a1, &a2, &a3, &a4, &a5, pt_cmrg[0][gb_index_x][0]);
  ...
  for (k=1; k<=M; k++){
	...
	stocks[increment] = Sk;
	It[increment] = P;
	MC_inner_k<<<32,32,2*32*sizeof(float)>>>(P1, P2, Sk, k*dt*dt, P, dt, B, K, L, M, Ntraj, pt_cmrg, option_prices + increment, gb_index_x);
  }
  ...
  CMRG_set_d(&a0, &a1, &a2, &a3, &a4, &a5, pt_cmrg[0][gb_index_x][0]);
}
...
{	
	...
	MC_outer_k<<<32,32,2*32*sizeof(float)>>>(P1, P2, S0, dt, B, K, leng, M, Ntraj, pt_CMRG, option_price_GPU, sum_GPU, option_prices_GPU, St_GPU, It_GPU);
