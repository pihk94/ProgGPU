...
//Inner trajectory
__global__ void MC_inner_k(int P1, int P2, float St,float _t, int It, float dt,float B, float K, int L, int M,int Ntraj, TabSeedCMRG_t *pt_cmrg,float *payoff,int index_rng){
...
__global__ void MC_k(int P1, int P2, float x_0, float dt, 
  ...
  CMRG_get_d(&a0, &a1, &a2, &a3, &a4, &a5, pt_cmrg[0][0][gb_index_x]);
  ...
  for (k=1; k<=M; k++){
	...
	Stocks[increment] = Sk;
	It[increment] = P;
	MC_inner_k<<<NbInner,NbInner,2*NbInner*sizeof(float)>>>(P1, P2, Sk, k*dt*dt, P, dt, B, K, L, M, NbInner*NbInner,pt_cmrg, payoffs + increment,gb_index_x);
  ...
  CMRG_set_d(&a0, &a1, &a2, &a3, &a4, &a5, pt_cmrg[0][0][gb_index_x]);
}
...
{	
	...
	MC_k<<<NbOuter,NbOuter,2*NbOuter*sizeof(float)>>>(P1, P2, x_0, dt, B, K, 
										  leng, M, NbOuter*NbOuter, pt_CMRG,
